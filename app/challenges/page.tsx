...
import { Layers, ShieldCheck, ArrowRight, FileText } from 'lucide-react';
import Link from 'next/link';

export default function ChallengesPage() {
  return (
    <div className="min-h-screen bg-slate-50 flex flex-col font-sans text-slate-900" id="challenges-page">
      ...
      <div className="bg-slate-900 text-white py-8 px-4 border-b border-slate-800">
        <div className="max-w-[1440px] mx-auto">
          <div className="flex items-center gap-2 text-xs text-slate-400 mb-2">
            ...
            <span>/</span>
            <span className="text-amber-400 font-semibold">Active Innovation Challenges</span>
          </div>
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h1 className="text-2xl sm:text-3xl font-black tracking-tight text-white flex items-center gap-3">
                <Layers className="w-7 h-7 text-sangam-saffron-400" />
                <span>Active Innovation Challenges</span>
              </h1>
              <p className="text-xs sm:text-sm text-slate-300 mt-1 max-w-3xl leading-relaxed">
                Explore statutory procurement problem statements published by Central Ministries, State Governments, and CPSEs with pre-allocated pilot budgets under GFR 2017 Rules 149 & 161(iv).
              </p>
            </div>
