...
export default function StartupDashboard({ preselectedChallenge }: StartupDashboardProps) {
  ...
  return (
    <div className="space-y-8">
      ...
      <div className="bg-gradient-to-r from-sangam-navy-900 to-sangam-blue-600 text-white rounded-2xl p-6 sm:p-8 shadow-md">
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6">
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2 pt-1 text-[11px] text-slate-300 font-medium">
              <span>📍 {currentStartup.city}, {currentStartup.state}</span>
              <span>•</span>
              <span>🏢 Team: {currentStartup.teamSize} Members</span>
              <span>•</span>
              <span>🌐 {currentStartup.website}</span>
              <span>•</span>
            ...
            <div className="space-y-6">
              {myProposals.map(prop => (
                ...
                >
                  ...
                  {/* Gemini AI Strategic Review Modal/Drawer if open for this proposal */}
                  {geminiAnalysisResult && geminiAnalysisResult.proposalId === prop.id && (
                    <div className="p-5 bg-gradient-to-r from-blue-50 to-indigo-50 rounded-xl border border-blue-200 text-xs space-y-3 animate-in fade-in duration-200">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-sangam-navy-900 text-sm flex items-center gap-1.5">
                          <Sparkles className="w-4 h-4 text-indigo-600" />
                          Gemini AI Government Committee Review
                        </span>
                        <button
              ...
              <div key={pilot.id} className="bg-white rounded-2xl border border-slate-200 p-6 sm:p-8 shadow-sm space-y-6">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-200 pb-4">
                  <div>
                    ...
                    <p className="text-xs text-slate-600 mt-1">📍 Corridor: {pilot.location}</p>
                  </div>
                  <div className="text-right">
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="w-full h-3 rounded-full bg-slate-100 overflow-hidden">
                    <div
                      className="h-full bg-gradient-to-r from-sangam-blue-600 to-emerald-500 rounded-full transition-all duration-500"
